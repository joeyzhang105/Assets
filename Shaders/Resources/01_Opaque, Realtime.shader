...
Shader "Mechanist/01 - Opaque, Realtime"
{
	Properties
	{
	...
	Subshader
	{
		...
		Pass
		{
			...
			Varys vert ( VertInput v )
			{
				...
				// lighting
               	float3 worldNormal = normalize ( mul ( v.normal, _World2Object ).xyz );
				o.sun = max ( 0, dot ( _WorldSpaceLightPos0, worldNormal ) ) * 1.5;
			...
			fixed4 frag ( Varys i ) : SV_Target {
				...
				fixed shadows = _Gbl_Amb + _Gbl_Lgt * i.sun;
				fixed4 outcolor = tex2D ( _MainTex, i.uv1 );
				outcolor.rgb *= shadows;
